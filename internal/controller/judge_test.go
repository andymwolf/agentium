...
func TestParseJudgeVerdict(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "SIMPLE verdict",
			output:       "Analysis...\nAGENTIUM_EVAL: SIMPLE straightforward change",
			wantVerdict:  VerdictSimple,
			wantFeedback: "straightforward change",
			wantSignal:   true,
		},
		{
			name:         "COMPLEX verdict",
			output:       "AGENTIUM_EVAL: COMPLEX requires architectural changes across multiple files",
			wantVerdict:  VerdictComplex,
			wantFeedback: "requires architectural changes across multiple files",
			wantSignal:   true,
		},
		{
			name:         "NOMERGE verdict",
			output:       "AGENTIUM_EVAL: NOMERGE low confidence, needs human review",
			wantVerdict:  VerdictNoMerge,
			wantFeedback: "low confidence, needs human review",
			wantSignal:   true,
		},
		{
