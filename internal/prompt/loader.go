...
// LoadProjectPrompt reads AGENTS.md from the given workspace directory root.
// Returns empty string with nil error if the file does not exist.
func LoadProjectPrompt(workDir string) (string, error) {
	agentMDPath := filepath.Join(workDir, "AGENTS.md")
...
// LoadProjectPromptWithPackage loads and merges root and package-specific AGENTS.md files.
...
func LoadProjectPromptWithPackage(workDir, packagePath string) (string, error) {
	...
	// Load package AGENTS.md (optional)
	if packagePath != "" {
